...
export function productYears(p: AnyProduct): ProductYear[] {
  ...
  const { months, years } = recurringBase(p);
...
export function productYear1Months(p: AnyProduct): number[] {
  if (!recurring(p)) {
...
export function productYear1Clients(p: AnyProduct): number[] {
  return recurring(p) ? recurringBase(p).months.slice(0, 12).map((m) => m.active) : [];
}

function recurringBase(p: AnyProduct) {
  const first = Math.min(5, Math.max(0, (num(p.start_selling_year) || 1) - 1));
  const byYear = newByYear(p);
  const monthly = Array.from({ length: 12 }, (_, i) => num(p.monthly_new_clients?.[String(i + 1)]));
  const entered = monthly.reduce((a, b) => a + b, 0);
  // Year 1 uses the month-by-month acquisition when it has been set; otherwise the year's count, spread evenly.
  const y1 = first > 1 ? Array(12).fill(0) : entered > 0 ? monthly : Array(12).fill(byYear[0] / 12);
  return recurringProjection({
    monthlyPrice: 1,                       // priced outside, so the year's fee rise can be applied
    openingClients: first > 1 ? 0 : num(p.opening_clients),
...
export function planRevenueByYear(products: AnyProduct[]) {
  ...
  for (const p of products) productYears(p).forEach((y, i) => { totals[i] += y.revenue; });
...
export function planYear1Months(products: AnyProduct[]) {
  ...
  for (const p of products) productYear1Months(p).forEach((v, i) => { totals[i] += v; });
