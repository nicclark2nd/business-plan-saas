...
type Row = Person & { _key: string; _state?: "saving" | "saved" | "error"; _error?: string };
...
export function PeopleModule({ planId, initial, currency }: { planId: string; initial: PeopleData; currency: string }) {
  ...
  /** Debounced save of a person's own fields (the row and the Salary/Productivity tabs). */
  const savePerson = (key: string, changes: Partial<Person>, immediate = false) => {
    setPeople((ps) => ps.map((r) => (r._key === key ? { ...r, ...changes, _state: undefined } : r)));
    clearTimeout(timers.current[key]);
    const run = () => {
      const row = peopleRef.current.find((r) => r._key === key); if (!row || !row.first_name.trim()) return;
      patch(key, { _state: "saving" });
      start(async () => {
        const res = await upsertPerson(planId, { ...row, id: row.id || undefined });
        if (res.ok) patch(key, { id: res.data!.id, _state: "saved" }); else patch(key, { _state: "error", _error: res.error });
      });
    };
    if (immediate) run(); else timers.current[key] = setTimeout(run, 800);
  };
  ...
  const saveItem = (kind: ListKind, item: AnyItem, changes: Record<string, unknown>) => {
    const next = { ...item, ...changes };
    setItems(kind, (xs) => xs.map((x) => (x.id === item.id ? next : x)));
    const k = `${kind}:${item.id}`; clearTimeout(timers.current[k]);
  ...
  return (
    <>
      ...
      <div className="rounded-md border border-border bg-card">
        <div className="flex items-center justify-between border-b border-border px-3 py-2.5">
          ...
          <span className="text-xs text-muted-foreground">
            {people.length} {people.length === 1 ? "person" : "people"} · rows save as you type
        ...
        {people.map((r) => (
          <div key={r._key} data-row={r._key} onClick={() => setSelected(r._key)}
            ...
            <div className="flex items-center justify-end gap-1">
              <span className="w-14 text-right text-[11px] text-muted-foreground">{r._state === "saving" ? "Saving…" : r._state === "error" ? <span className="text-bad" title={r._error}>Error</span> : r.id ? "Saved" : ""}</span>
      ...
      {sel && sel.id && (
        <div className="mt-3 rounded-md border border-border bg-card">
          <div className="flex items-center justify-between border-b border-border px-4 py-3">
            ...
            <span className="text-xs text-muted-foreground">Optional detail — everything here is saved as you type</span>
          </div>
          <Tabs defaultValue="salary" className="px-4 pb-4 pt-2">
            ...
            <TabsContent value="salary" className="pt-3">
              <SalaryTab person={sel} currency={currency} onChange={(c) => savePerson(sel._key, c)} />
            </TabsContent>
            ...
            <TabsContent value="duties" className="pt-3">
              <ItemList title="Duties" hint="One responsibility per line — what this person is accountable for." onAdd={() => addItem("duties", sel.id, { duty: "" })} empty={items("duties", sel.id).length === 0}>
                {items("duties", sel.id).map((it) => (
                  <ItemRow key={it.id} id={it.id} onRemove={() => removeItem("duties", it)}>
            ...
            <TabsContent value="qualities" className="pt-3">
              <ItemList title="Qualities" hint="Skills, strengths, expertise, certifications — and areas for development." onAdd={() => addItem("qualities", sel.id, { kind: "skill", description: "" })} empty={items("qualities", sel.id).length === 0}>
                {items("qualities", sel.id).map((it) => (
                  <ItemRow key={it.id} id={it.id} onRemove={() => removeItem("qualities", it)} lead={
                    <Select value={String(it.kind)} onValueChange={(v) => v && saveItem("qualities", it, { kind: v })}><SelectTrigger className="w-[150px]"><SelectValue /></SelectTrigger><SelectContent>{QUALITY_KINDS.map((k) => <SelectItem key={k} value={k}>{cap(k)}</SelectItem>)}</SelectContent></Select>}>
            ...
            <TabsContent value="education" className="pt-3">
              <ItemList title="Education & training" hint="Degrees, certifications, courses, workshops — with where and when." onAdd={() => addItem("education", sel.id, { kind: "course", institution: "", year_completed: "", description: "" })} empty={items("education", sel.id).length === 0}>
                {items("education", sel.id).map((it) => (
                  <ItemRow key={it.id} id={it.id} onRemove={() => removeItem("education", it)} lead={
                    <Select value={String(it.kind)} onValueChange={(v) => v && saveItem("education", it, { kind: v })}><SelectTrigger className="w-[150px]"><SelectValue /></SelectTrigger><SelectContent>{EDUCATION_KINDS.map((k) => <SelectItem key={k} value={k}>{cap(k)}</SelectItem>)}</SelectContent></Select>}>
            ...
            <TabsContent value="focus" className="pt-3">
              <ItemList title="Focus this year" hint="The one to three things this person will concentrate on, with a priority and a target date." onAdd={() => addItem("focus", sel.id, { focus_area: "", description: "", priority: "medium", target_date: null })} empty={items("focus", sel.id).length === 0}>
                {items("focus", sel.id).map((it) => (
                  <ItemRow key={it.id} id={it.id} onRemove={() => removeItem("focus", it)} lead={
                    <Select value={String(it.priority)} onValueChange={(v) => v && saveItem("focus", it, { priority: v })}><SelectTrigger className={cn("w-[120px]", it.priority === "high" && "text-bad", it.priority === "medium" && "text-warn")}><SelectValue /></SelectTrigger><SelectContent>{PRIORITIES.map((k) => <SelectItem key={k} value={k}>{cap(k)}</SelectItem>)}</SelectContent></Select>}>
      ...
      <form id="people-form" action={(fd) => { Object.values(timers.current).forEach(clearTimeout); start(() => continueFromPeople(planId, fd.get("intent") === "next" ? "next" : "later")); }} />
    </>
  );
}

function SalaryTab({ person, currency, onChange }: { person: Person; currency: string; onChange: (c: Partial<Person>) => void }) {
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-[1fr_200px] gap-4">
        ...
        <div className="space-y-1.5"><Label>Starts in</Label>
          <Select value={String(person.salary_start_year)} onValueChange={(v) => v && onChange({ salary_start_year: Number(v) })}><SelectTrigger className="w-full"><SelectValue /></SelectTrigger><SelectContent>{SALARY_YEARS.map((y) => <SelectItem key={y} value={String(y)}>Year {y}</SelectItem>)}</SelectContent></Select></div>
      </div>
      <div>
...
function ItemRow({ id, lead, children, onRemove }: { id: string; lead?: React.ReactNode; children: React.ReactNode; onRemove: () => void }) {
  return (
    <div data-item={id} className="flex items-start gap-2 rounded-md border border-border bg-secondary/40 p-2">
      {lead}
