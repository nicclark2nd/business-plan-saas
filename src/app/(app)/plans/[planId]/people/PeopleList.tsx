...
export function PeopleList({ planId, initial }: { planId: string; initial: Person[] }) {
  ...
  const save = (i: number, form: HTMLFormElement) => {
    ...
    start(async () => {
      ...
      else patch(i, { id: res.id!, _new: false, _open: false, _dirty: false, _error: undefined, name: String(fd.get("name")), position: String(fd.get("position") ?? ""),
    ...
    <>
      <div className="rounded-md border border-border bg-card">
        ...
        <div className="max-h-[520px] overflow-auto">
          {rows.length > 0 && (
            <div className="sticky top-0 z-[1] grid grid-cols-[minmax(160px,1.4fr)_minmax(150px,1.2fr)_100px_130px_120px] gap-3 border-b border-input bg-card px-4 py-2 text-[11px] font-semibold uppercase tracking-[0.05em] text-muted-foreground whitespace-nowrap">
          ...
          {rows.map((r, i) => (
            <div key={r.id || `new-${i}`} className={cn("border-b border-border last:border-b-0", r._new && "bg-accent/40")}>
              {!r._open ? (
                <div className="grid grid-cols-[minmax(160px,1.4fr)_minmax(150px,1.2fr)_100px_130px_120px] items-center gap-3 px-4 py-2.5 text-[13px]">
