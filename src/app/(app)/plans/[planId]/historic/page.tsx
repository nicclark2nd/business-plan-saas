...
export default async function HistoricPage({ params, searchParams }: { params: Promise<{ planId: string }>; searchParams: Promise<{ area?: string }> }) {
  ...
  const loadError = periods.error?.message ?? settings.error?.message ?? null;
  if (loadError) console.error("historic load", periods.error ?? settings.error);
  ...
  return (
    <HistoricModule planId={planId} initial={rows} hasHistory={settings.data?.has_history ?? null} mode={mode}
      initialArea={area === "bs" || area === "import" ? area : "pnl"} fyEndMonth={settings.data?.financial_year_end_month ?? 6} loadError={loadError} />
  );
}
