 ...
 */
export function GoalsModule({ planId, mode, initial, people, quarters, thisQuarter }: {
  ...
  const setAnnual = (a: GoalArea, title: string) =>
    setGoals((xs) => {
      ...
      return [...xs, { id: `tmp-${a}`, parent_id: null, area: a, title, detail: null, year: null, quarter: null, owner_person_id: null, status: "not_started", milestone_date: null, source: "manual", sort_order: 0 }];
    });
...
function QuarterlyDialog({ area, goal, people, quarters, pending, onClose, onSave }: {
  area: GoalArea; goal?: Goal; people: Person[]; quarters: QuarterChoice[]; pending: boolean;
  onClose: () => void;
  onSave: (input: { title: string; year: number; quarter: number; ownerPersonId: string | null; status: GoalStatus; milestoneDate: string | null }) => void;
}) {
  ...
  return (
    <Dialog open onOpenChange={onClose}>
      <DialogContent className="max-w-[560px]">
        <DialogHeader>
          <DialogTitle>{goal ? "Edit" : "New"} quarterly goal · {AREA_LABEL[area]}</DialogTitle>
        </DialogHeader>
        ...
        <DialogFooter>
          ...
          <Button disabled={pending || !title.trim()}
            onClick={() => onSave({ title, year, quarter, ownerPersonId: owner || null, status, milestoneDate: milestone || null })}>
