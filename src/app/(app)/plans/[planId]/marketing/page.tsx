...
export default async function MarketingPage({ params, searchParams }: { params: Promise<{ planId: string }>; searchParams: Promise<{ area?: string }> }) {
  ...
  const [session, market, competitors, spend, evidence, settings] = await Promise.all([
    getSession(),
  ...
  const initialArea = ["market", "competitors", "spend", "evidence"].includes(area ?? "") ? (area as "market") : "market";
