...
export default async function MarketingPage({ params, searchParams }: { params: Promise<{ planId: string }>; searchParams: Promise<{ area?: string }> }) {
  ...
  const [session, market, spend, evidence, settings, actions, people, products] = await Promise.all([
    getSession(),
    ...
    supabase.from("plan_settings").select("customer_type, financial_year_end_month, first_projected_year").eq("plan_id", planId).maybeSingle(),
    /**
  ...
  return <MarketingModule planId={planId} initial={data} mode={mode} initialArea={initialArea}
    customerWord={(settings.data?.customer_type ?? "customer").toLowerCase()}
